...
def run_all_auto_resolves() -> dict[str, Any]:
    """Resolve open bets across main, LM, NO, +EV, CS, H2H, and Arahus logs."""
    ...
    for log_type in ("main", "lm", "no", "ev", "cs", "h2h", "arahus", "arahus_v2", "arahus_live_v1"):
        try:
...
def start_auto_resolve_scheduler() -> BackgroundScheduler | None:
    ...
    if _env_flag("AUTO_RESOLVE_SCHEDULE_ENABLED", default=True):
        try:
            ...
            logger.info(
                "Scheduled auto-resolve at %s :%02d %s (main + lm + no + ev + cs + h2h + arahus + arahus_v2 + arahus_live_v1)",
